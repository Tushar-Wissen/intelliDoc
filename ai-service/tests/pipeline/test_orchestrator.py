...
from tests.fakes import FakeMinio, InMemoryPipelineRepository, OverlapEmbeddingModel
...
def test_orchestrator_indexes_chunks_and_defers_ready(monkeypatch):
    ...
    assert repo.get_document(document_id).processing_status == "INDEXING"
    assert repo.jobs[document_id].status == "RUNNING"
