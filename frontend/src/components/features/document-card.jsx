...
export function DocumentCard({ document, index = 0, selected = false, onClick }) {
  ...
  const pages = estimatePageCount(document.content);
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group flex flex-col items-start gap-3 rounded-xl border bg-card p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md',
    ...
    >
      <div className="relative">
        ...
        {statusDot && (
          <span
            className={cn('absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full ring-2 ring-card', statusDot)}
      ...
      <div className="mt-1 flex w-full items-center gap-1.5 border-t border-border pt-3 text-xs text-muted-foreground">
